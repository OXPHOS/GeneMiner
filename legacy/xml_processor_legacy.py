...
def process_xml():
    ...
    if psql:
        filelist = list(database_connector.psql_file_loader(spark, tbname="xml_list").collect())
    else:
        filelist = list(database_connector.redshift_file_loader(spark, tbname="xml_list", tmpdir="xml_files").collect())
